    ...
    while True:
        for i in range(step):
            ...
            for j in range (begin,end):
                if(X_train[j].shape[0]>=batch_max_len):
                    batch_max_len=X_train[j].shape[0]
            np_x_data=np.zeros((batch_size,batch_max_len,256),dtype=np.float32)
            np_y_data=np.zeros((batch_size,batch_max_len,80),dtype=np.float32)
            k=-1
            for j in range (begin,end):
                k=k+1
                if(X_train[j].shape[0]<batch_max_len):
                    np_x_data[k]=np.pad(X_train[j],((0,batch_max_len-X_train[j].shape[0]),(0,0)),'constant')
                    np_y_data[k]=np.pad(Y_train[j],((0,batch_max_len-X_train[j].shape[0]),(0,0)),'constant')
                else:
                    np_x_data[k]=X_train[j][:,:]
                    np_y_data[k]=Y_train[j][0:batch_max_len,:]
...
#     inputs=layers.Dense(units=128)(input_data)
    ...
    highway_net=Highway(highway_net)
    """
    gru_output=layers.Bidirectional(layers.GRU(256, return_sequences=True))(highway_net)
...
checkpoint_dir = '/home/team06/week3_code/WT/model_final_256/CBHGweights.best.h5'
if os.path.exists(checkpoint_dir):
    ...
    model = load_model('./model_final_256/CBHGweights.best.h5',custom_objects={"Highway":Highway,"layers":layers,"k_init":k_init})
else:
...
filepath="./model_final_256/cbhgmodel_{epoch:02d}-{loss:.2f}.h5"
...
filebestmodel="./model_final_256/CBHGweights.best.h5"
checkpoint2= ModelCheckpoint(filebestmodel, monitor='loss', verbose=1,save_best_only=True,mode='min',period=1)
#tensorboard
callbacks_list = [checkpoint,checkpoint2,TensorBoard(log_dir="./logs/CBHG_final_256")]
#训练
history=model.fit_generator(load_input_generator(Batch_size,X_train,Y_train),steps_per_epoch=len(X_train)//Batch_size,epochs=1000,verbose=1,callbacks=callbacks_list,validation_data=(X_test,Y_test),initial_epoch=119)
