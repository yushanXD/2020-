...
#             print(self.istarget)
            ...
            self.mean_loss = tf.reduce_mean(tf.reduce_sum(self.loss * self.istarget,reduction_indices=[1,2]))
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('tensorboard/lm/02', tf.get_default_graph())
    min_loss = 500000000
    cnt = 0
    for k in range(epochs):
        ...
        if (cnt + 1) % checkpoint_steps == 0:
            if av_loss < min_loss:
                min_loss = av_loss 
                saver.save(sess, './logs/model.ckpt', global_step=cnt+1)
